...
MKFS_DEVTABLE_OPT := -D $(BUILD_DIR)/device_table.txt
...
SQUASHFS_COMP := xz
ifeq ($(CONFIG_KERNEL_SQUASHFS_ZLIB),y)
	SQUASHFS_COMP := gzip
endif

ifeq ($(CONFIG_KERNEL_SQUASHFS_XZ),y)
	SQUASHFS_COMP := xz
endif

ifeq ($(CONFIG_KERNEL_SQUASHFS_LZ4),y)
	SQUASHFS_COMP := lz4
endif
...
multi-partitions := $(foreach p,\
			$(subst $(quote),,$(CONFIG_IMAGE_TO_MULTI_PARTITIONS)),\
			$(patsubst %/,%,$(p))$(sep)) root
...
define Image/mkfs/squashfs-common
	$(STAGING_DIR_HOST)/bin/mksquashfs4 $(1) $(KDIR)/$(2) -noappend -root-owned -comp $(SQUASHFS_COMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),$(MKSQUSHFS_CPU_NUM))
endef

# $(1): input file or directory. (as default, it will TARGET_DIR,
#                  but if we define the Image/Input. It will be passed as intput file or directory)
# $(2): output file. (file name foramt: partition.filetype, such as root.squashfs)
#
define Image/mkfs/squashfs
	@mkdir -p $(1)/overlay
ifneq ($(CONFIG_SUNXI_SMALL_STORAGE_OTA),)
	$(call Aw/BuildUpgradeImage/normal-prepare,$(1),./temp_usr)
	$(STAGING_DIR_HOST)/bin/mksquashfs4 ./temp_usr $(KDIR)/usr.squashfs -noappend -root-owned -comp $(SQUASHFS_COMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),$(MKSQUSHFS_CPU_NUM))
endif
ifneq ($(CONFIG_PACKAGE_sepolicy-demo)$(CONFIG_PACKAGE_refpolicy),)
	echo "$(STAGING_DIR)/host/sbin/setfiles -r $(1) $(1)/etc/selinux/targeted/contexts/files/file_contexts $(1)" > $(TARGET_OUT_DIR)/.fakeroot-script
	echo "$(Image/mkfs/squashfs-common)" >> $(TARGET_OUT_DIR)/.fakeroot-script
	chmod +x $(TARGET_OUT_DIR)/.fakeroot-script
	$(STAGING_DIR_HOST)/bin/fakeroot $(TARGET_OUT_DIR)/.fakeroot-script
else
	$(call Image/mkfs/squashfs-common,$(1),$(2))
endif
ifneq ($(CONFIG_SUNXI_SMALL_STORAGE_OTA),)
	$(call Aw/BuildUpgradeImage/normal-resume,$(1),./temp_usr)
endif
endef
...
ifneq ($(CONFIG_NAND_SUPPORT),)
   define Image/Build/SysupgradeNAND
	...
	(cd "$(KDIR_TMP)"; $(TAR) cvf \
		"$(TARGET_OUT_DIR)/$(IMG_PREFIX)-$(1)-$(2)-sysupgrade.tar" sysupgrade-$(1) \
...
define Image/mkfs/cpiogz
	( cd $(TARGET_DIR); find . | cpio -o -H newc | gzip -9n >$(TARGET_OUT_DIR)/$(IMG_PREFIX)-rootfs.cpio.gz )
endef

define Image/mkfs/targz
	$(TAR) -cp --numeric-owner --owner=0 --group=0 --sort=name \
		$(if $(SOURCE_DATE_EPOCH),--mtime="@$(SOURCE_DATE_EPOCH)") \
		-C $(TARGET_DIR)/ . | gzip -9n > $(TARGET_OUT_DIR)/$(IMG_PREFIX)$(if $(PROFILE_SANITIZED),-$(PROFILE_SANITIZED))-rootfs.tar.gz
endef
...
define Image/mkfs/prepare/default
	...
	chmod 751 $(TARGET_DIR)/
	chmod 751 $(TARGET_DIR)/bin $(TARGET_DIR)/dev $(TARGET_DIR)/etc $(TARGET_DIR)/proc $(TARGET_DIR)/rom $(TARGET_DIR)/root $(TARGET_DIR)/sbin
...
ifneq ($(CONFIG_SUNXI_SEPARATE_APP_FROM_ROOTFS),)
	...
	mv $(TARGET_DIR)/mnt/app $(TARGET_DIR)/../app
	mkdir -p $(TARGET_DIR)/mnt/app
endif
ifneq ($(CONFIG_SUNXI_SEPARATE_DATA_FROM_ROOTFS),)
	rm -rf $(TARGET_DIR)/../data
	mkdir -p $(TARGET_DIR)/data
	mv $(TARGET_DIR)/data $(TARGET_DIR)/../data
	mkdir -p $(TARGET_DIR)/data
endif
endef
...
# $(1): filesystem type
# $(2): partition name (option)
#
#     Before call Image/mkfs/{filetype}, will call Hooks/mkfs/Pre,
#     that it can crete the image filter scripts to process the partition files.
#
#     And, also cat define the Image/Input to change the input directory of this
#     partition image.
#
define BuildImage/mkfs
  install: mkfs-$(1)$(if $(strip $(2)),-$(2))
  .PHONY: mkfs-$(1)$(if $(strip $(2)),-$(2))
  mkfs-$(1)$(if $(strip $(2)),-$(2)): kernel_prepare
	$(foreach hook,$(Hooks/mkfs/Pre),$(call $(hook),$(1),$(2))$(sep))
	$(call Image/mkfs/$(1),$(if $(Image/Input),$(Image/Input),$(TARGET_DIR)),$(2).$(1))
	$(call Build/mkfs/default,$(1))
	$(call Build/mkfs/$(1),$(1))
	$(foreach hook,$(Hooks/mkfs/Post),$(call $(hook),$(1),$(2))$(sep))
  $(KDIR)/$(if $(strip $(2)),$(strip $(2)),root).$(1): mkfs-$(1)$(if $(strip $(2)),-$(2))
  $(eval Image/Input:=)
...
define Device/Build/initramfs
  ...
  $$(_TARGET): $(TARGET_OUT_DIR)/$$(KERNEL_INITRAMFS_IMAGE)

  $(KDIR)/$$(KERNEL_INITRAMFS_NAME):: image_prepare
  $(TARGET_OUT_DIR)/$$(KERNEL_INITRAMFS_IMAGE): $(KDIR)/tmp/$$(KERNEL_INITRAMFS_IMAGE)
	cp $$^ $$@
...
define Device/Build/kernel
  ...
  $$(_TARGET): $$(if $$(KERNEL_INSTALL),$(TARGET_OUT_DIR)/$$(KERNEL_IMAGE))
  $(call Device/Export,$(KDIR)/$$(KERNEL_IMAGE),$(1))
  $(TARGET_OUT_DIR)/$$(KERNEL_IMAGE): $(KDIR)/$$(KERNEL_IMAGE)
	cp $$^ $$@
  ifndef IB
    ifdef CONFIG_IB
...
define Device/Build/image
  $$(_TARGET): $(TARGET_OUT_DIR)/$(call IMAGE_NAME,$(1),$(2))
  ...
  .IGNORE: $(TARGET_OUT_DIR)/$(call IMAGE_NAME,$(1),$(2))
  $(TARGET_OUT_DIR)/$(call IMAGE_NAME,$(1),$(2)): $(KDIR)/tmp/$(call IMAGE_NAME,$(1),$(2))
	cp $$^ $$@
...
define BuildImage
  ...
  kernel_prepare: mkfs_prepare
	$(if $(CONFIG_TARGET_ROOTFS_INITRAMFS),$(if $(IB),,$(call Image/BuildKernel/Initramfs)), $(call Image/BuildKernel))
	$(call Image/InstallKernel)

  $(foreach device,$(TARGET_DEVICES),$(call Device,$(device)))
  $(foreach fs,$(TARGET_FILESYSTEMS) $(fs-subtypes-y),\
	  $(foreach p,$(multi-partitions),$(eval $(call BuildImage/mkfs,$(fs),$(p)))))
